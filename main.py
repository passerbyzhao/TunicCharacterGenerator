...
def get_args_parser():
    ...
    parser.add_argument("--sentence",action=StripArgument,  help='想要翻译的句子')
    ...
    parser.add_argument("--fontsize", type=int, default=15, help='指定字体大小 和通常意义上的字号不相通')
    parser.add_argument("-p", "--path", help='图片输出位置 默认为根目录')
    parser.add_argument('-n', '--name', default='result', help='输出图片的名称')
    parser.add_argument('--pictype', choices=['.jpg'], default='.jpg', help='实验功能 选择输出图片格式')
    parser.add_argument('--pensize', default=4, type=int, help='笔画的粗细')
...
def FullFuction(sentence, font_size, style='formal', pensize=4, pencolor='black'):
    if style == 'formal':
    ...
    mid.checker()
    phoneme_list = mid.wordtophoneme()
    # print(phoneme_list)
...
if __name__ == '__main__':
    ...
    # parser.parse_args(['--sentence', ' hello world, i\'m you. how are you? '])
    args = parser.parse_args()
    tt.pencolor(args.pencolor)
    # args.sentence = ' hello world, i\'m you. how are you? '
    # sen = ' hello world'
    # sen = 'banana'
    w, h, iW, iH, W, H = FullFuction(args.sentence, args.fontsize, args.mode, args.pensize, args.pencolor)
    # print(scale)
